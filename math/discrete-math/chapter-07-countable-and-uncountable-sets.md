...
This chapter extends the idea of size from finite sets to infinite sets.

For finite sets, size is just the number of elements.

For infinite sets, the chapter uses a different idea:
...
This chapter is one of the first places where infinity stops being a vague word and becomes a structured mathematical object.
...
The chapter starts by revisiting bijections.
...
So the chapter uses an alternating enumeration:
...
The chapter then counts ordered pairs of natural numbers.
...
The chapter notes an important theorem:
...
The chapter’s main uncountability example is:
...
The chapter writes the contradiction in the classic form:
...
The chapter then gives the general comparison language.
...
The chapter hints at the general statement:
...
One of the best parts of the chapter is the connection to computability.
...
The chapter closes by pointing to the halting problem.
...
That makes this chapter especially important for theoretical computer science.

## Worked Exercise Patterns

The main payoff of this chapter is learning to classify infinite sets by the right encoding or comparison argument:

- explicit bijection,
- diagonal sweep,
- countable union,
- power-set blowup,
- or direct diagonalization.

### 7.1. There are countably many different infinite sizes

The clean pattern is the iterated power-set ladder:
...
and each `A_n` is infinite.

The reusable lesson is:

```text
power sets generate a strictly increasing hierarchy of infinite cardinalities.
```

### 7.2. Finite strings over a finite alphabet

The set of all finite strings over a finite alphabet is countable because:

- strings of each fixed length form a finite set,
- and the whole set is a countable union of those finite layers.

The reusable lesson is:

```text
finite alphabet + finite strings = countable.
```

### 7.3 and 7.6. Diagonalization is list-independent

The objection in 7.3 fails because diagonalization is not about one missing object from one bad list.
It is about a procedure that defeats every proposed list.

The real-number proof in 7.6 uses the same structure:

- assume a list,
- alter the diagonal digits,
- produce a new object not on the list.

The reusable lesson is:

```text
diagonalization is a universal anti-enumeration method,
not a patchable gap in one specific list.
```

### 7.4 and 7.5. Infinite set arithmetic is unlike finite arithmetic

These exercises show that for infinite sets:

- removing a subset can leave nothing,
- or finitely many elements,
- or a countably infinite set,
- or a set of the same size as the original.

They also show that:

- countable unions can be countably infinite,
- uncountable differences can leave countable sets,
- and intersections of uncountable sets can even be empty.

The reusable lesson is:

```text
finite-set size intuition is unreliable once infinite sets enter.
```

### 7.7 and 7.8. Pairing and interleaving

There are two main pairing patterns:

- diagonal pairing for `N × N`,
- digit interleaving for pairs of reals in `[0,1]`.

Both prove that “two coordinates” do not necessarily mean a larger cardinality.

The reusable lesson is:

```text
to show two infinite sets have the same size,
look for an explicit coding scheme.
```

### 7.9. Complexity of a description is not the same as cardinality

The “books” problem separates several ideas:

- finite strings over a finite alphabet are countable,
- bounded-length strings are finite,
- finite subsets of a countable set are countable,
- but power sets of countable sets are uncountable.

The reusable lesson is:

```text
cardinality depends on the structure of the encoding space,
not on whether the objects sound rich or complicated.
```

### 7.10. Finite subsets of N

Enumerating finite subsets by size fails because each size layer is already infinite.

Enumerating them by sum of elements works because each fixed-sum layer is finite.

The reusable lesson is:

```text
an infinite family is countable only if the chosen layering actually lets you leave each layer in finite time.
```

### 7.11. Countable union of countable sets

This theorem is the row-by-row diagonal sweep pattern:

- one countable set per row,
- countably many rows,
- diagonal traversal through `N × N`.

The reusable lesson is:

```text
countably many countable rows still fit into one countable grid.
```

### 7.12. Function spaces depend critically on the domain

Functions:

- `{0,1} -> N`

are just ordered pairs of naturals, so they are countable.

Functions:

- `N -> {0,1}`

are infinite binary sequences, so they are uncountable by diagonalization.

The reusable lesson is:

```text
finite domain + countable codomain behaves like finite tuples,
but infinite domain + even a 2-point codomain already explodes to uncountable size.
```
...
Countably infinite sets are the infinite sets that can be listed by natural numbers.
Uncountable sets cannot be fully enumerated, and diagonalization proves that power sets like `P(N)` and binary-sequence spaces like `N -> {0,1}` are strictly larger than `N`.

This chapter matters because it turns infinity into a structured hierarchy rather than a single vague concept.
