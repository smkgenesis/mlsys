...
This chapter matters because it turns earlier set ideas into a language for:
...
The chapter also emphasizes that `A` and `B` do not need to be disjoint. For example:
...
The chapter shows several ways a relation can fail to be a function.
...
The chapter briefly introduces the idea of a partial function.
...
The chapter notes an important consequence:
...
The chapter gives several counting consequences when `A` and `B` are finite.
...
The chapter explains that a function with several arguments is still just a single-argument function whose domain is a Cartesian product.
...
This chapter really depends on the previous sets chapter.
...
The main payoff of this chapter is learning to classify what kind of mapping problem you are looking at:

- a relation versus a function,
- inverse relation versus inverse function,
- injective/surjective/bijective classification,
- finite-set counting consequences,
- or function composition and its inverse behavior.

### 6.1. When the inverse relation is a function

If the inverse relation `f^(-1)` is a function, then the original function `f` must be injective.

That immediately means:

- each output in `image(f)` came from exactly one input,
- so `f^(-1)` is a function from `image(f)` to `dom(f)`,
- and in fact it is bijective there.

The reusable lesson is:

```text
inverse relation becomes a genuine function exactly when the original map is one-to-one on its image.
```

### 6.2. Classifying concrete functions

The point of the injective/surjective exercises is not only to memorize definitions.
It is to build the habit of checking three separate things:

1. can two different inputs collide?
2. does every codomain element get hit?
3. if injective but not onto, what is the inverse on the image?

The reusable lesson is:

```text
always test injectivity and surjectivity separately;
they are logically independent.
```

### 6.3. Finite versus infinite sets

If `A` and `B` are finite and the same size, then:

- injective `A -> B` already forces surjective,
- and surjective already forces injective.

But this fails for infinite sets.
...
```text
finite sets tie injective and surjective tightly together;
infinite sets separate them.
```

### 6.4. The inverse of the circle relation
...
but the equation is symmetric in `x` and `y`, so:
...
```text
inverse relation means flip ordered pairs,
then check whether the defining condition is symmetric.
```

### 6.5. Inverse of a composition

For `f ∘ g` to make sense, you need:
...
Then `(f ∘ g)^(-1)` exists as a function exactly when `f ∘ g` is injective.
...
```text
inverse of a composition reverses order.
```

### 6.6. Counting by functions and the Extended Pigeonhole Principle

The computer-time problem is a function-counting argument in disguise.

Each person contributes 7 different seven-hour blocks containing their one-hour slot, so there are:

```text
15 · 7 = 105
```

inputs to a function whose outputs are only the 24 possible seven-hour blocks in a day.

Extended Pigeonhole then forces some block to be hit at least:

```text
ceil(105 / 24) = 5
```

times, and distinct parameter choices for one person cannot give the same block.

The reusable lesson is:

```text
build a function whose codomain is small enough that repeated hits force the desired structure.
```

### 6.7. Gluing bijections on disjoint pieces

If:

- `f(n) = 2n` bijects `Z` with the even integers,
- `g(n) = 2n + 1` bijects `Z` with the odd integers,

then the inverse maps are:
...
and a piecewise function can recover the inverse on the union by splitting into even and odd cases.

The reusable lesson is:

```text
bijections on disjoint pieces can often be stitched together by a case split on the codomain.
```
...
This chapter is one of the first places where “mathematical object” starts looking a lot like “typed interface with constraints.”

## Short Takeaway

Relations are subsets of Cartesian products, and functions are the special relations that assign each input exactly one output.

From there, image, inverse relation, injective, surjective, and bijective all describe different ways a mapping can behave.
This chapter matters because it turns set language into a language of structure-preserving connections.
